...
> ⚠ **Раздел заменён и сохранён как история.** Источник истины —
> `BandKit_Chat_and_Messaging_Security_v1.md` (заменяет этот §8) и
> `BandKit_Conversation_Lifecycle_and_Abuse_Controls_v1.md`.
>
> Что здесь больше не верно: **E2EE by default для личных чатов**. Текущие чаты работают
> в режиме `server_managed` — иначе невозможны server-side Link Guard, moderation
> evidence и поиск. E2EE выведен в отдельный будущий epic только для личных диалогов;
> чаты сущностей остаются `server_managed` по дизайну. Заявлять E2EE в UI или
> документации до реализации полного протокола запрещено (`CLAUDE.md`).
>
> Классов переписки ровно два: канонический личный диалог на пару пользователей и чат,
> принадлежащий ровно одной сущности. Перечисленных ниже типов комнат в модели нет.
