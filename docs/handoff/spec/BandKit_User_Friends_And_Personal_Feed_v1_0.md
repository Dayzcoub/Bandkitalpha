...
> **Не источник истины с 2026-07-17 (F2, 1.25.0).** Оси, значения, дефолты и хранение —
> `BandKit_Privacy_Model_v1`. Список ниже сохранён как продуктовое пожелание, которым он и
> был: в MVP реализованы **две** оси (`dm`, `friend_request`), потому что у остальных нет
> поверхности, которая бы их применяла — маршрута профиля пользователя не существует,
> личных постов нет, `last_seen` нет. Каждая появится вместе со своим экраном, строкой в
> справочнике. Не реализовывать этот список как ТЗ и не считать его отставанием.
>
> Отдельно: `friends of friends` из «Suggested options» не заведён — двухшаговый обход
> графа без единого потребителя. `circle` у `friend_request` невозможен by design (модель
> §4): тот, кто уже в круге, заявку не шлёт.
