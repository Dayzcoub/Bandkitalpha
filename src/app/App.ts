...
function chatHistoryChrome(ctx: AppContext): string {
  ...
  const pinnedHtml = pinned ? `<section class="bk-chat-pinned-summary" data-pinned-message-id="${pinned.id}" data-pinned-at="${pinned.pinnedAt}" data-pinned-by="${pinned.pinnedBy}"><div><span>${pinned.meta}</span><strong>${pinned.title}</strong><p>${pinned.body}</p></div>${pinnedAction}</section>` : '';
  return `<details class="bk-chat-history-dropdown"><summary><span>${meta.title}</span><small>Поиск и фильтры</small></summary><section class="bk-chat-history-toolbar" aria-label="Навигация по истории чата"><div class="bk-chat-history-search"><span aria-hidden="true">⌕</span><input type="search" aria-label="Поиск в чате" placeholder="Поиск в чате, документах и решениях" /></div><div class="bk-chat-history-filters">${filters}</div>${pinnedHtml}</section></details><button class="bk-chat-load-older" type="button">Загрузить старые сообщения</button><div class="bk-chat-date-divider">18 мая 2026</div>`;
}
