...
export function qaControlCard(ctx: AppContext): string {
  ...
  if (!canSeeDiagnostics(ctx)) {
    return card(`<details class="bk-qa-details"><summary>${ctx.t('rightRail.rolePreview')}</summary><div class="bk-qa-grid">${roleSwitcher}</div></details>`, 'bk-rail-card bk-qa-card');
  }
...
export function safetyRailCard(ctx: AppContext): string {
  const diagnostics = canSeeDiagnostics(ctx) ? `<div class="bk-chip-row">${badge(ctx.t('security.auditReady'), 'positive')}${badge(ctx.t('security.reportAvailable'), 'warning')}</div>` : '';
  return card(`<h3 class="bk-card-title">${ctx.t('rightRail.safetyTitle')}</h3><div class="bk-trust-list">${trustChecks.map((check) => trustCheckCard(ctx, check)).join('')}</div>${diagnostics}`, 'bk-rail-card');
}
